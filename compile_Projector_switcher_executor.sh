...
upgrade_lib_dir=$linux_project_dir/upgrade
...
if [ -f $eagle_flash_file ]; then
	cp -f $eagle_flash_file $windows_project_dir/bin/
	echo "$eagle_flash_file file copied"
fi

if [ -f $eagle_irom_file ]; then
	cp -f $eagle_irom_file $windows_project_dir/bin/
	echo "$eagle_irom_file file copied"
fi
